...
void SetupTimer2(void)
{
	...
	// https://www.youtube.com/watch?v=bu6TTZHnMPY
	__builtin_disable_interrupts();
	//PR2 = (SYSCLK / 5);  // since SYSCLK/FREQ = PS*(PR1+1)
	TMR2 = 0;
...
// 	while(len--) wait_1ms();
// }
#define MEASURE_REF 1
#define PIN_PERIOD (PORTB&(1<<4))
...
void main(void)
{
	...
	ANSELB &= ~(1 << 4); // Set RB5 as a digital I/O
	TRISB |= (1 << 4);   // configure pin RB5 as input
	CNPUB |= (1 << 4);   // Enable pull-up resistor for RB5


	waitms(500);
	printf("Period measurement using the core timer free running counter.\r\n"
		"Connect signal to RB4 (pin 11).\r\n");
	ref_freq = Get_Ref_Freq();
	//T2CONbits.ON = 1;

	while (1) {

		if (!MEASURE_REF) ref_freq = Get_Ref_Freq();

		live_freq = live_frequency();
		freq_diff = live_freq - ref_freq;
		//PR2 = (SYSCLK / (1+freq_diff/5));
